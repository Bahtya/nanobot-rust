...
use kestrel_session::{Note, Session};
...
pub fn extract_compaction_notes(
    session: &mut Session,
    old_messages: &[kestrel_session::SessionEntry],
) -> usize {
    ...
    let topics: Vec<String> = old_messages
        .iter()
        .filter(|m| m.role == kestrel_core::MessageRole::User)
    ...
    let action_items: Vec<String> = old_messages
        .iter()
        .filter(|m| m.role == kestrel_core::MessageRole::Assistant)
    ...
    let decisions: Vec<String> = old_messages
        .iter()
        .filter(|m| {
            m.role == kestrel_core::MessageRole::Assistant
                || m.role == kestrel_core::MessageRole::User
        })
        .flat_map(|m| m.content.lines())
        .filter(|line| {
            let lower = line.to_lowercase();
...
mod tests {
    ...
    fn test_extract_compaction_notes_action_items() {
        let mut session = make_session();
        let messages = vec![kestrel_session::SessionEntry {
            role: kestrel_core::MessageRole::Assistant,

...
//!
//! Loaded from `~/.kestrel/skills.toml` (or `KESTREL_HOME/skills.toml`).
...
pub struct SkillConfig {
    ...
    ///
    /// Defaults to `$KESTREL_HOME/skills` or `~/.kestrel/skills`.
...
fn default_skills_dir() -> PathBuf {
    kestrel_home().join("skills")
}

fn kestrel_home() -> PathBuf {
    std::env::var("KESTREL_HOME")
        .map(PathBuf::from)
        .unwrap_or_else(|_| dirs::home_dir().unwrap_or_default().join(".kestrel"))
}
